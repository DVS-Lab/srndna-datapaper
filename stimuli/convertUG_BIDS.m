...
try
    ...
    for r = 0:1
        ...
        fid = fopen(myfile,'w');
        fprintf(fid,'onset\tduration\ttrial_type\tresponse_time\tOffer\tIsFairBlock\n');
  ...
  if subj_gen==0 and subj_eth==0 and subj_age > 35:
    ...
    '1': 'computer',
            ...
            if response(t) == 2
                fprintf(fid,'%f\t%f\t%s\t%f\t%d\t%d\n',onset(t),duration(t),['event_reject_' ptype],RT(t),Offer(t),IsFairBlock(t));
            elseif response(t) == 3
                fprintf(fid,'%f\t%f\t%s\t%f\t%d\t%d\n',onset(t),duration(t),['event_accept_' ptype],RT(t),Offer(t),IsFairBlock(t));
            elseif response(t) == 999
                fprintf(fid,'%f\t%f\t%s\t%s\t%d\t%d\n',onset(t),duration(t),'missed_trial','n/a', Offer(t),IsFairBlock(t));
            end
            ...
            if response(t) == 2 || response(t) == 3 % only valid responses
                ...
                fprintf(fid,'%f\t%f\t%s\t%f\t%d\t%d\n',onset(t),duration(t),['event_' ptype],RT(t),Offer(t),IsFairBlock(t));
            end
            
            block_starts = [1 9 17 25 33 41 49 57 65];
            if ismember(t,block_starts)
                fprintf(fid,'%f\t%f\t%s\t%s\t%s\t%s\n',onset(t),33.5,['block_' trial_type],'n/a','n/a','n/a');
            else
                if response(t) == 2 || response(t) == 3 % only valid responses
                    ...
                    fprintf(fid,'%f\t%d\t%s\t%f\t%d\t%d\n',onset(t),0,'event_RT',RT(t),Offer(t),IsFairBlock(t));
                end
            end
